...
astcenc_error astcenc_config_init(
	...
	else
	{
		...
		// luminance the usual weights to compute luminance from an RGB value are as follows:
		//
		//     l = r * 0.3 + g * 0.59 + b * 0.11
		//
		// ... but we scale these up to keep a better balance between color and alpha.
		if (flags & ASTCENC_FLG_USE_PERCEPTUAL)
		{
			config.cw_r_weight = 0.3f * 2.0f;
			config.cw_g_weight = 0.59f * 2.0f;
			config.cw_b_weight = 0.11f * 2.0f;
		}
	}
